...
export interface MapConfigParams {
  ...
  /** 各区负荷数据 */
  districtLoads: Record<string, number>
}
...
/**
 * 构建北京电网态势感知3D地图的 ECharts 配置
 *
 * @param params - 地图配置参数
 * @param params.mode - 当前显示模式：电网/负荷/告警
 * @param params.substations500 - 500kV变电站列表
 * @param params.substations220 - 220kV变电站列表
 * @param params.substations110 - 110kV变电站列表
 * @param params.transmissionLines - 输电线路列表
 * @param params.districtLoads - 各区负荷数据
...
export function buildMapOption({
  ...
  transmissionLines,
  districtLoads
}: MapConfigParams): echarts.EChartsOption {
