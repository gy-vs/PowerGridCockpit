...
import 'echarts-gl'
import { getMapOption, type MapMode, type Substation, type TransmissionLine } from '../configs/mapConfig'
...
const mapModes = [
  { key: 'grid' as MapMode, label: '电网' },
  { key: 'load' as MapMode, label: '负荷' },
  { key: 'alarm' as MapMode, label: '告警' }
]
...
function switchMode(key: MapMode) {
...
watch(activeMode, () => {
  if (chart && mapLoaded) {
    chart.setOption(getMapOption({
      mode: activeMode.value,
      substations500: SUBSTATIONS_500,
      substations220: SUBSTATIONS_220,
      substations110: SUBSTATIONS_110,
      transmissionLines: TRANSMISSION_LINES,
      districtLoads: DISTRICT_LOADS
    }), true)
  }
})
