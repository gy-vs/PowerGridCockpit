...
import { getEnergyConsumptionOption } from '../configs/energyConsumptionConfig'
...
onMounted(() => {
  ...
  chart.setOption(getEnergyConsumptionOption({ districts: DISTRICTS, values: getConsumption() }))
  timer = setInterval(() => {
    chart?.setOption(getEnergyConsumptionOption({ districts: DISTRICTS, values: getConsumption() }))
  }, 6000)
